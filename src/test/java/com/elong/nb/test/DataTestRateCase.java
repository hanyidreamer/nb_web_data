...
public class DataTestRateCase {
	private static SimpleDateFormat sdf = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	private static String url1 = "http://10.88.25.65:8971/OpenApiWeb/api/Hotel/GetRates";
	private static String url2 = "http://10.39.21.82:8971/OpenApiWeb/api/Hotel/GetRates";
	...
	static void readHtml() {
		...
		String json = HttpUtil
				.httpPost(
						...
						"{\"size\":200,\"query\":{\"query_string\":{\"query\":\"methodName:getRates\",\"use_dis_max\":true}}}",
						null);
		...
		for (int i = 0; i < jArray.size(); i++) {
			...
			String str = param.replace('[', ' ').replace(']', ' ').trim()
					.replace("\\", "");
			;
...
//				Map<String, Inventory> map2 = getMap(response2);
				...
				if (!is) {
					errorCount++;
					File file = new File("rate_result.txt");
	...
	static boolean compare(Map<String, Inventory> map1,
			Map<String, Inventory> map2, String p1, String p2) {
		boolean result = true;
		for (String key : map1.keySet()) {
			...
			if (map2.containsKey(key)) {
				...
				if (!(inv2.isStatus() == inv1.isStatus())) {
					result = false;
					record.append("Status " + p1 + inv1.isStatus() + p2
							+ inv2.isStatus());
					record.append("\t");
				...
				if (!(inv2.getAvailableAmount() == inv1.getAvailableAmount())) {
					result = false;
					record.append("AvailableAmount" + p1
							+ inv1.getAvailableAmount() + p2
							+ inv2.getAvailableAmount());
					record.append("\t");
				...
				// }
				if (!(inv2.getEndDate().getTime() == inv1.getEndDate()
						.getTime() || inv2.getEndDate().getTime() <= 0)) {
				...
				if (!(inv2.getEndTime().equals(inv1.getEndTime()))) {
					result = false;
					record.append("EndTime" + p1 + inv1.getEndTime() + p2
							+ inv2.getEndTime());
					record.append("\t");
				...
				if (!(inv2.getOverBooking() == inv1.getOverBooking())) {
					result = false;
					record.append("OverBooking" + p1 + inv1.getOverBooking()
							+ p2 + inv2.getOverBooking());
					record.append("\t");
					// System.out.println("key"+key+"getOverBooking() not same"+" url1:"+inv1.getOverBooking()+" url2:"+inv2.getOverBooking()+" v:"+log);
				}
				if (!(inv2.getStartDate().getTime() == inv1.getStartDate()
						.getTime() || inv2.getStartDate().getTime() <= 0)) {
					 result=false;
...
//					 System.out.println("key"+key+"getStartDate() not same"+" url1:"+sdf.format(inv1.getStartDate())+" url2:"+sdf.format(inv2.getStartDate()));
				...
				if (!(inv2.getStartTime().equals(inv1.getStartTime()))) {
					result = false;
					record.append("Status" + p1 + inv1.getStartTime() + p2
							+ inv2.getStartTime());
					record.append("\t");
					// System.out.println("key"+key+"getStartTime() not same"+" url1:"+inv1.getStartTime()+" url2:"+inv2.getStartTime());
				}
				if (!((inv2.isIsInstantConfirm() == null && inv1
						.isIsInstantConfirm() == null)
						|| (inv1.isIsInstantConfirm() != null && inv1
								.isIsInstantConfirm().equals(
										inv2.isIsInstantConfirm())) || (inv2
						.isIsInstantConfirm() != null && inv2
						.isIsInstantConfirm().equals(inv1.isIsInstantConfirm())))) {
					result = false;
					record.append("IsInstantConfirm" + p1
							+ inv1.isIsInstantConfirm() + p2
							+ inv2.isIsInstantConfirm());
					record.append("\t");
				...
				if (!((inv2.getIC_BeginTime() == null && inv1.getIC_BeginTime() == null)
						|| (inv2.getIC_BeginTime() != null && inv2
								.getIC_BeginTime().equals(
										inv1.getIC_BeginTime())) || (inv1
						.getIC_BeginTime() != null && inv1.getIC_BeginTime()
						.equals(inv2.getIC_BeginTime())))) {
					result = false;
					record.append("IC_BeginTime" + p1 + inv1.getIC_BeginTime()
							+ p2 + inv2.getIC_BeginTime());
					record.append("\t");
					// System.out.println("key"+key+"getIC_BeginTime() not same"+" url1:"+inv1.getIC_BeginTime()+" url2:"+inv2.getStartDate());
				}
				if (!((inv2.getIC_EndTime() == null && inv1.getIC_EndTime() == null) || (inv2
						.getIC_EndTime() != null && inv2.getIC_EndTime()
						.equals(inv1.getIC_EndTime() != null
								&& inv1.getIC_EndTime().equals(
										inv2.getIC_EndTime()))))) {
					result = false;
					record.append("IC_EndTime" + p1 + inv1.getIC_EndTime() + p2
							+ inv2.getIC_EndTime());
					record.append("\t");
	...
	static Map<String, Inventory> getMap(RestResponse<InventoryResult> response1) {
		...
		if (response1 != null && response1.getResult() != null
				&& response1.getResult().getInventories() != null) {
			for (Inventory inv : response1.getResult().getInventories()) {
				map1.put(
						inv.getHotelID() + "|" + inv.getHotelCode() + "|"
								+ inv.getRoomTypeID() + "|"
								+ sdf1.format(inv.getAvailableDate()), inv);
			}
		}
		return map1;
	}
	...
	public static <T> RestResponse<T> toResponse(String resultJson,
		...
		try {
			...
			// if (res != null && res.getGuid() != null &&
			// res.getGuid().length() >
			// 0)
			// ra.setAttribute(Constants.ELONG_REQUEST_REQUESTGUID,
			// req.getGuid(),
	...
	@SuppressWarnings("rawtypes")
	public static <T> RestRequest<T> toReq(String request, Class<T> clazz,
			Map<Class, TypeAdapter> adapters) throws IOException {
		String json = request;
		...
		// if (req != null && req.getGuid() != null && req.getGuid().length() >
		// 0)
