...
public class DataTestRate {
		...
		if (response1 != null && response1.getResult() != null && response1.getResult().getRates() != null) {
			for (Rate rate : response1.getResult().getRates()) {
				...
				while (start.getTime() < end.getTime()) {
					map1.put(rate.getHotelID() + "|" + rate.getHotelCode() + "|" + rate.getRoomTypeId() + "|" + rate.getRateplanId() + "|"
							+ sd.format(start), rate);
	...
	public static <T> RestResponse<T> toResponse(String resultJson, Type typeObj, Map<Class, TypeAdapter> adapters) throws IOException {
