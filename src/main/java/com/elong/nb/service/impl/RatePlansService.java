...
//    	condition.setProductType("1");
    ...
    //	ArrayOfHotelDetail response = northBoundForAPIService.searchHotelRatePlanList(condition);
    		SearchHotelRatePlanListResp response=new SearchHotelRatePlanListResp();
        List<HotelRatePlan> result = new LinkedList<HotelRatePlan>();

        if(response==null || response.getResult()==null )
        	return result;
        
        for (HotelDetail hotel : response.getResult())
        {
        	...
        	{
        	  rp.setHotelID(mHotelId == null ? mSRelationRepository.GetMHotelId(hotel.getHotelBaseInfo().getShotelId()) : mHotelId);
        	}
    ...
    private List<RatePlan> getRatePlans(HotelDetail hotel, EnumLocal language, ProxyAccount proxyInfo, double requestVersion, String options)
    {
        ...
        if (requestVersion >= 1.27) 
        {
            ...
            else 
            {
            	...
            	{
	                //获取合作类型，是艺龙直签还是其它供应商
	            	MSHotelRelation hotelRelation = M_SRelationRepository.getHotelRelation(hotel.getHotelBaseInfo().getShotelId());
	                if (hotelRelation != null)
	                {
	                    int type = mSRelationRepository.getCooperationTypeBySupplierID(hotelRelation.getSupplierId());
	                    //CooperationType=1为直签，2为非直签，0为未知
  ...
  /// <summary>
    ...
    private List<com.elong.nb.model.bean.base.BasePrepayRule> getPrepayRules(RatePlanBaseInfo oldrp, EnumLocal language)
    {
        ...
        for (PrePayInfo rule : oldrp.getRatePlanPrePayRuleList())
        {
            ...
            if(rule.getPrepayChangeRule() == EnumPrepayChangeRule.PrepayNeedOneTime.getValue())
            	prepayrule = EnumPrepayChangeRule.PrepayNeedOneTime;
            else if(rule.getPrepayChangeRule() == EnumPrepayChangeRule.PrepayNeedSomeDay.getValue())
            ...
            else if(rule.getCutTypeAfter()==EnumPrepayCutPayment.Percent.getValue())
            ...
            else if(rule.getCutTypeBefor()==EnumPrepayCutPayment.Percent.getValue())
    ...
    private List<com.elong.nb.model.bean.base.BaseValueAddRule> getValueAddRules(RatePlanBaseInfo oldrp, EnumLocal language)
    {
        ...
        for (AddValuePolicyInfo rule : oldrp.getAddValuePolicyList())
        {
        	...
        	{
	            cnDescription = DateUtil.getDateString(rule.getStartDate()) + " - " + DateUtil.getDateString(rule.getEndDate()) + ((rule.getIsInclude() == 0) ? " 不含早餐" : " 包含 " + rule.getShare() + " 份早餐");
	            enDescription = DateUtil.getDateString(rule.getStartDate()) + " - " + DateUtil.getDateString(rule.getEndDate()) + ((rule.getIsInclude() == 0) ? " no breakfast" : " includes " + rule.getShare() + " breakfast");
        	}
    ...
    private List<SupplierRatePlan> getSuppliers(HotelDetail hotel, EnumLocal language)
    {
        ...
        SupplierRatePlan suprp = new SupplierRatePlan();
            ...
            if( hotel.getHotelBaseInfo() !=null)
            {
	            ...
	            List<MSRoomRelation> msList = mSRelationRepository.getMSRoomRelation(hotel.getHotelBaseInfo().getShotelId());
	...
	@Override
	public RestResponse<RatePlanResult> GetRatePlans(RestRequest<RatePlanCondition> request) {
