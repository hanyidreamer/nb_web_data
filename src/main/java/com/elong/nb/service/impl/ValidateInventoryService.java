...
public class ValidateInventoryService implements IValidateInventoryService {
	 ...
	 * 验证库存
	 */
	public RestResponse<ValidateInventoryResult> getValidateInventories(RestRequest<ValidateInventoryCondition> restRequest,
			ProxyAccount proxyAccount) throws Exception {
		...
		List<Inventory> inventories = this.inventoryService.getInventories(req, proxyAccount).getResult().getInventories();
			...
			if (q != null && q.size() > 0) {
				for (Inventory inv : q) {
					if (!inv.isStatus() // 库存状态
							...
							|| (restRequest.getRequest().getAmount() > 0 && inv.getOverBooking() == 1 && inv.getAvailableAmount() < restRequest
					...
					if (inv.getAvailableDate().getTime() == DateUtil.getDate(new Date()).getTime()
							|| inv.getAvailableDate().getTime() == DateUtil.getDate(DateUtil.addDays(new Date(), -1)).getTime()) {
