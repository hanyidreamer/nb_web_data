...
public class CommentService implements ICommentService {
	...
	@Override
	public RestResponse<CommentResult> ListComments(RestRequest<CommentCondition> request) {
		RestResponse<CommentResult> result = new RestResponse<CommentResult>(request.getGuid());
		if (request.getProxyInfo().getEnabledCommentReadRole() == false) {
			return result;
		}
		StringBuilder message = null;
        CommentResult res = commontRepository.list(request.getRequest().getHotelId(), request.getRequest().getPageSize(), request.getRequest().getPageIndex(),message, request.getProxyInfo().getCommentUserKey());
        if (message== null){
	...
	@Override
	public RestResponse<CommentSummaryResult> CommentSummary(RestRequest<CommentSummaryCondition> request){
         ...
         StringBuilder message = null;
         CommentSummaryResult res = commontRepository.getSumarries(request.getRequest().getHotelIds(), message, request.getProxyInfo().getCommentUserKey());
         if (message == null){
             result.setResult(res);
         }else{
