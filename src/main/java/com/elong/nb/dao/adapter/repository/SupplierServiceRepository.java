...
public class SupplierServiceRepository {
	...
	public GetSupplierInfoBySupplierIDResponse getSupplierInfoBySupplierID(GetSupplierInfoBySupplierIDRequest request){
		try{
			...
			ActionLogHelper.businessLog(UUID.randomUUID().toString(), true, "getSupplierInfoBySupplierID","ISupplierServiceContract",null, (System.currentTimeMillis()-start), 0, null,null,"");
			return res;
		}catch(Exception e){
			ActionLogHelper.businessLog(UUID.randomUUID().toString(), false, "getSupplierInfoBySupplierID","ISupplierServiceContract",e, 0, 0, e.getMessage(),null,"");
