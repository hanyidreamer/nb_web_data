...
public class DescriptionHelper {
	...
	public static String drrDescription(MetaDRRInfo metaDRRInfo, boolean isCn) {
		...
		case 1:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("提前")
						.append(metaDRRInfo.getRuleValues().get("DayNum")).append("天预订，每间晚优惠");
		...
		case 2:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("连住")
						.append(metaDRRInfo.getRuleValues().get("CheckInNum")).append("晚起，每间晚优惠");
				if ("1".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("元");
				} else if ("2".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("%");
		...
		case 3:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("连住")
						.append(metaDRRInfo.getRuleValues().get("CheckInNum")).append("晚起，最后")
						.append(metaDRRInfo.getRuleValues().get("LastDayNum")).append("晚优惠");
				if ("1".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("元");
				} else if ("2".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("%");
		...
		case 4:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("连住")
						.append(metaDRRInfo.getRuleValues().get("CheckInNum")).append("晚起，第")
						.append(metaDRRInfo.getRuleValues().get("WhichDayNum")).append("晚起优惠");
				if ("1".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("元");
				} else if ("2".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("%");
		...
		case 5:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("每连住")
						.append(metaDRRInfo.getRuleValues().get("EveryCheckInNum")).append("晚，最后")
						.append(metaDRRInfo.getRuleValues().get("LastDayNum")).append("晚优惠");
				if ("1".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
					res.append(metaDRRInfo.getRuleValues().get("DeductNum")).append("元");
				} else if ("2".equals(metaDRRInfo.getRuleValues().get("CashScale"))) {
		...
		case 6:
			if (isCn) {
				...
				res.append(dateType).append("在").append(sdf.format(new Date(metaDRRInfo.getStart_date()))).append("-")
						.append(sdf.format(new Date(metaDRRInfo.getEnd_date()))).append(", ").append("在店日期包含")
						.append(getWeekSet(metaDRRInfo.getIs_week_effective())).append("，按");
	...
	public static String prepayDescription(MetaPrePayInfo metaPrePayInfo, boolean isCn) {
		...
		case 1:
			if (isCn) {
				res.append("预付规则：在").append(sdf.format(new Date(metaPrePayInfo.getStart_date()))).append(" 到")
						.append(sdf.format(new Date(metaPrePayInfo.getEnd_date()))).append("期间入住，需要提供信用卡预付全额房费。一经预订成功不可变更/取消。");
			} else {
		...
		case 2:
			if (isCn) {
				res.append("预付规则：在").append(sdf.format(new Date(metaPrePayInfo.getStart_date()))).append(" 到")
						.append(sdf.format(new Date(metaPrePayInfo.getEnd_date()))).append("期间入住");
				...
				if (metaPrePayInfo.isCut_after_change_time()) {
					res.append("Cancellations made within ").append(metaPrePayInfo.getRule_values().get("HourNum"))
							.append(" hours before the check-in date are subject to penalty of ");
					if ("1".equals(metaPrePayInfo.getRule_values().get("CashScaleFirstAfter"))) {
				...
				res.append(" Within ").append(metaPrePayInfo.getRule_values().get("HourNum2"))
						.append("hours before the check-in date , bookings will be non-refundable. ");
			}
			break;
		case 3:
			if (isCn) {
				res.append("预付规则：在").append(sdf.format(new Date(metaPrePayInfo.getStart_date()))).append(" 到")
						.append(sdf.format(new Date(metaPrePayInfo.getEnd_date()))).append("期间入住");
				if (metaPrePayInfo.isCut_after_change_time()) {
					res.append("，在").append(metaPrePayInfo.getRule_values().get("DateNum")).append(" ")
							.append(metaPrePayInfo.getRule_values().get("TimeNum")).append("之后")
							.append(metaPrePayInfo.getRule_values().get("HourNum")).append("小时取消将收取罚金");
					if ("1".equals(metaPrePayInfo.getRule_values().get("CashScaleFirstAfter"))) {
						res.append(metaPrePayInfo.getRule_values().get("DeductNumAfter")).append("元");
					}
					if ("2".equals(metaPrePayInfo.getRule_values().get("CashScaleFirstAfter"))) {
				...
				if (metaPrePayInfo.isCut_after_change_time()) {
					res.append("Cancellations made within ").append(metaPrePayInfo.getRule_values().get("HourNum")).append(" between ")
							.append(metaPrePayInfo.getRule_values().get("DateNum")).append(" ")
							.append(metaPrePayInfo.getRule_values().get("TimeNum"))
							.append(" and the check-in date are subject to penalty of ");
					if ("1".equals(metaPrePayInfo.getRule_values().get("CashScaleFirstAfter"))) {
						res.append(metaPrePayInfo.getRule_values().get("DeductNumAfter")).append("RMB");
					}
					if ("2".equals(metaPrePayInfo.getRule_values().get("CashScaleFirstAfter"))) {
	...
	public static String vouchInfoDescription(MetaVouchInfo metaVouchInfo, boolean isCn, EnumGuaranteeMoneyType moneyType) {
		...
		case 1:
			if (isCn) {
				res.append("担保条件：在").append(sdf.format(new Date(metaVouchInfo.getStart_date()))).append("至")
						.append(sdf.format(new Date(metaVouchInfo.getEndDate())));
				if (metaVouchInfo.getDate_type() == 1) {
					res.append("入住");
				} else if (metaVouchInfo.getDate_type() == 2) {
					res.append("在店");
				}
				if (metaVouchInfo.isIs_arrive_time_vouch() && metaVouchInfo.isIs_room_count_vouch()) {
					res.append(" 如果在").append(metaVouchInfo.getArrive_start_time()).append("至").append(metaVouchInfo.getArrive_end_time())
							.append("到店，或预订房量超过").append(metaVouchInfo.getRoom_count()).append("间及以上");
				}
				if (metaVouchInfo.isIs_arrive_time_vouch() && !metaVouchInfo.isIs_room_count_vouch()) {
					res.append(" 在").append(metaVouchInfo.getArrive_start_time()).append("至" + metaVouchInfo.getArrive_end_time())
							.append("到店");
				}
				if (!metaVouchInfo.isIs_arrive_time_vouch() && metaVouchInfo.isIs_room_count_vouch()) {
					res.append(" 如果预订房量超过").append(metaVouchInfo.getRoom_count()).append("间及以上");
				}
				String vouchType = "";
				if (metaVouchInfo.getRule_values() != null && metaVouchInfo.getRule_values().containsKey("Gage")) {
				...
				String removeNextDayEndTime = metaVouchInfo.getArrive_end_time().replaceAll("次日", "");
				if (!metaVouchInfo.isIs_arrive_time_vouch() && !metaVouchInfo.isIs_room_count_vouch()) {
					res.append("For Bookings with  check-in date between ").append(sdf.format(new Date(metaVouchInfo.getStart_date())))
							.append(" and ").append(sdf.format(new Date(metaVouchInfo.getEndDate()))).append(", ");
				}
				if (metaVouchInfo.isIs_arrive_time_vouch() && !metaVouchInfo.isIs_room_count_vouch()) {
					res.append("For Bookings with  check-in time between ").append(metaVouchInfo.getArrive_start_time()).append(" and ")
							.append(removeNextDayEndTime).append(", ");
				}
				if (!metaVouchInfo.isIs_arrive_time_vouch() && metaVouchInfo.isIs_room_count_vouch()) {
					res.append("For bookings with ").append(metaVouchInfo.getRoom_count()).append("  rooms or above booked, ");
				}
				if (metaVouchInfo.isIs_arrive_time_vouch() && metaVouchInfo.isIs_room_count_vouch()) {
					res.append("For bookings with ").append(metaVouchInfo.getRoom_count())
							.append("  rooms or above booked, or check-in time between ").append(metaVouchInfo.getArrive_start_time())
							.append(" and ").append(removeNextDayEndTime).append(", ");
				}
		...
		case 2:
			if (isCn) {
				...
				res.append(
						...
						.append(metaVouchInfo.getRule_values().get("TimeNum")).append(", ")
		...
		case 3:
			if (isCn) {
				...
				if (metaVouchInfo.isIs_arrive_time_vouch() && metaVouchInfo.isIs_room_count_vouch()) {
					res.append(" 如果在").append(metaVouchInfo.getArrive_start_time()).append("至").append(metaVouchInfo.getArrive_end_time())
							.append("到店，或预订房量超过").append(metaVouchInfo.getRoom_count()).append("间");
				}
				if (metaVouchInfo.isIs_arrive_time_vouch() && !metaVouchInfo.isIs_room_count_vouch()) {
					res.append(" 在").append(metaVouchInfo.getArrive_start_time()).append("至" + metaVouchInfo.getArrive_end_time())
							.append("到店");
				...
				res.append(
						...
						.append(metaVouchInfo.getRule_values().get("HourNum")).append(" hours before your earliest arrival time")
		...
		case 4:
			if (isCn) {
				...
				res.append(
						"credit card information is required for guarantee. Once the booking is confirmed, free cancellation or change is allowed until ")
						.append(metaVouchInfo.getRule_values().get("HourNum")).append(" hours before your check-in date").append(", ")
						.append(metaVouchInfo.getRule_values().get("DayNum"))
						.append(".If you fail to check in, eLong is authorized to charge ");
				String vouchMoneyType = "";
				if (moneyType == EnumGuaranteeMoneyType.FirstNightCost) {
	...
	/** 
	 *  1）过滤礼包:同时含“点评”和“艺龙”两个字样
	 *	2）替换关键词:将 “艺龙网” “艺龙” “elong” 替换为 “本站” 
	 *
	 * @param giftContentCn
	 * @return
	 */
	public static String giftContentFilter(String giftContentCn) {
		if (StringUtils.contains(giftContentCn, "点评") && StringUtils.contains(giftContentCn, "艺龙"))
			return null;
		if (giftContentCn.toLowerCase().contains("elong")) {
