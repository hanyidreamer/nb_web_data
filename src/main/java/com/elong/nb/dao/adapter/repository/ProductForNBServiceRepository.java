...
public class ProductForNBServiceRepository {
	...
	public GetRatePlanBaseInfoResponse2 getRatePlanBaseInfo(GetRatePlanBaseInfoRequest request){
		try{
			...
			ActionLogHelper.businessLog(UUID.randomUUID().toString(), true, "getRatePlanBaseInfo","IProductForNBServiceContract",null, (System.currentTimeMillis()-start), 0, null,null,"");
			return res;
		}catch(Exception e){
			ActionLogHelper.businessLog(UUID.randomUUID().toString(), false, "getRatePlanBaseInfo","IProductForNBServiceContract",e, 0, 0, e.getMessage(),null,"");
