...
public class InventoryThread implements Callable<List<Inventory>> {
	...
	public InventoryThread(RestRequest<BookingDataCondition> request, IInventoryService inventoryRepository,
			boolean isInstantConfirmInSearch, ProxyAccount proxyAccount) {
	...
	public List<Inventory> call() throws Exception {
		...
		try {
			...
			RestResponse<InventoryResult> res = inventoryRepository.getInventoriesForBooking(restRequest, proxyAccount);
